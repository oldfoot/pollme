...
class TriviaQuestionAnswers {
	...
	function __construct() {
		...
		$this->params_add = array("questionid","answer","ordering","correctanswer");
		$this->vartypes_add = array("questionid"=>"numeric","answer"=>"any","ordering"=>"numeric","correctanswer"=>"any");
		...
		$this->params_del = array("questionid");
		$this->vartypes_del = array("questionid"=>"numeric");
		$this->errors = "";
	...
	public function Add() {
		...
		// DOES THS USER EXIST?
		$sql = "call sp_trivia_question_answer_add($sp_params)";
		$this->debug($sql);
		...
		if (!$result) {
			...
			if (ISSET($this->suppressresponse) && $this->suppressresponse == "y") {
				// DON'T SHOW ANY RESPONSE TO AVOID DUPLICATES WHEN ADDING TO QUESTIONS
			}
			else {
				$this->Errors(MessageCatalogue(79));
			}
			return true;				
		}
		$this->Errors(MessageCatalogue(80));		
	}
	...
	public function Delete() {
		...
		$sp_params = substr($sp_params,0,-1);
		// DOES THS USER EXIST?
		$sql = "call sp_trivia_question_answer_delete($sp_params)";
		$this->debug($sql);
		$result = $GLOBALS['db']->Query($sql);
		if (!$result) {
