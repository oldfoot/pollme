...
class PollQuestionUserAnswers {
	...
	function __construct() {
		...
		$this->params_add = array("userid","answerid");
		$this->vartypes_add = array("userid"=>"any","answerid"=>"numeric");
	...
	public function Add() {
		...
		// DOES THS USER EXIST?
		$sql = "call sp_poll_question_user_answers_add($sp_params)";
		$this->debug($sql);
		$result = $GLOBALS['db']->Query($sql);
		if (!$result) {
			$this->Errors(MessageCatalogue(108));
			return false;				
		}
		else {
			...
			$this->Errors(MessageCatalogue(107));
			return true;				
		}
		$this->Errors(MessageCatalogue(108));		
	}
	...
	public function Delete() {
		...
		$sp_params = substr($sp_params,0,-1);
		// DOES THS USER EXIST?
		$sql = "call sp_poll_user_answers_del($sp_params)";
		$this->debug($sql);
		$result = $GLOBALS['db']->Query($sql);
		if (!$result) {
