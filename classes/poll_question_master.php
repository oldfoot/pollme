...
class PollQuestionMaster {
	...
	function __construct() {
		...
		$this->params_add = array("question","questiontype","useridadded","friendlycode","status","answerlimit","public");
		$this->vartypes_add = array("question"=>"any","questiontype"=>"any","useridadded"=>"numeric","friendlycode"=>"any","status"=>"any","answerlimit"=>"numeric","public"=>"any");
		$this->params_edit = array("pollid","question","useridedited","friendlycode","status","answerlimit","public");
		$this->vartypes_edit = array("pollid"=>"numeric","question"=>"any","useridedited"=>"numeric","friendlycode"=>"any","status"=>"any","answerlimit"=>"numeric","public"=>"any");
		$this->params_del = array("pollid","userid");
		$this->vartypes_del = array("pollid"=>"numeric","userid"=>"numeric");
		$this->errors = "";		
		$this->debug  = false;
	}
	...
	public function Info() {
		if (!ISSET($this->pollid) || !IS_NUMERIC($this->pollid)) { $this->Errors("Invalid Params"); return false; }
		$db=$GLOBALS['db'];
		$sql="CALL sp_poll_question_master_browse('".$this->pollid."')";
		$this->debug($sql);		
		$result = $db->Query($sql);
	...
	public function Add() {
		...
		if ($this->PollCodeExists()) {
			$this->debug("Poll Code Exists");
			//$this->Errors("Invalid data");
			return false;
		}
		...
		// DOES THS USER EXIST?
		$sql = "call sp_poll_question_master_add($sp_params)";
		$this->debug($sql);
	...
	public function Edit() {
		...
		$sp_params = substr($sp_params,0,-1);
		// DOES THS USER EXIST?
		$sql = "call sp_poll_question_master_edit($sp_params)";
		$this->debug($sql);
		$result = $GLOBALS['db']->Query($sql);
		if (!$result) {
	...
	public function Delete() {
		...
		$sp_params = substr($sp_params,0,-1);
		// DOES THS USER EXIST?
		$sql = "call sp_poll_question_master_del($sp_params)";
		$this->debug($sql);
		$result = $GLOBALS['db']->Query($sql);
		if (!$result) {
	...
	public function PollCodeExists() {
		if (!ISSET($this->friendlycode)) {
			$this->debug("Failed to provide correct params [1]");
			return false;
		}
		$sql = "call sp_poll_question_master_friendlycode_exists('".$this->friendlycode."')";
		$this->debug($sql);
		$result = $GLOBALS['db']->Query($sql);
		if ($GLOBALS['db']->AffectedRows($result) > 0) {
			$this->Errors(MessageCatalogue(109));
			return True;		
		}
		//$this->Errors(MessageCatalogue(98));
		return false;				
	}
	public function PollCodeExistsEdit() {
		if (!ISSET($this->friendlycode) || !ISSET($this->pollid)) {
			$this->debug("Failed to provide correct params [1]");
			return false;
		}
		$sql = "call sp_poll_question_master_friendlycode_exists_edit('".$this->friendlycode."','".$this->pollid."')";
		$this->debug($sql);
		$result = $GLOBALS['db']->Query($sql);
		if ($GLOBALS['db']->AffectedRows($result) > 0) {
			$this->Errors(MessageCatalogue(109));
